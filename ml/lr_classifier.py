...
def lr_classifier():
    ...
    if not os.path.exists(os.path.join('ml','lr_vectors.pkl')):
        ...
        pickle.dump(combine_vectors, open(os.path.join('ml','lr_vectors.pkl'), 'wb'))
    else:
        print('Loading pre-trained vectors...')
        combine_vectors = pickle.load(open(os.path.join('ml','lr_vectors.pkl'), 'rb'))
    ...
    return classify(train_vectors, train_labels, test_vectors, test_labels, type='LR', penalty='', pkl_classifier=r'lr_classifier.pkl')
